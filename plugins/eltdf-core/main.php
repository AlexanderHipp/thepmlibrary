...
if(!function_exists('eltd_core_activation')) {
     ...
     * and defines superfood_elated_core_on_activate action
     */
    function eltd_core_activation() {
        do_action('superfood_elated_core_on_activate');
...
if(!function_exists('eltd_core_theme_menu')) {
    ...
    function eltd_core_theme_menu() {
        if (eltd_core_theme_installed()) {
            ...
            superfood_elated_init_theme_options();
            ...
            $page_hook_suffix = add_menu_page(
                ...
                'superfood_elated_theme_menu',                // The ID used to bind submenu items to this menu
                array($superfood_elated_global_Framework->getSkin(), 'renderOptions'), // The callback function used to render this menu
                $superfood_elated_global_Framework->getSkin()->getMenuIcon('options'),             // Icon For menu Item
                $superfood_elated_global_Framework->getSkin()->getMenuItemPosition('options')            // Position
            );

            foreach ($superfood_elated_global_Framework->eltdOptions->adminPages as $key=>$value ) {
                $slug = "";
                ...
                $subpage_hook_suffix = add_submenu_page(
                    ...
                    array($superfood_elated_global_Framework->getSkin(), 'renderOptions')
                );

                add_action('admin_print_scripts-'.$subpage_hook_suffix, 'superfood_elated_enqueue_admin_scripts');
                add_action('admin_print_styles-'.$subpage_hook_suffix, 'superfood_elated_enqueue_admin_styles');
            };

            add_action('admin_print_scripts-'.$page_hook_suffix, 'superfood_elated_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$page_hook_suffix, 'superfood_elated_enqueue_admin_styles');
        }
    }
