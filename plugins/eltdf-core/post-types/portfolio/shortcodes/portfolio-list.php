...
class PortfolioList implements Lib\ShortcodeInterface {
		    ...
		    vc_map( array(
				    ...
				    'params'                    => array(
					    ...
					    array(
						    ...
						    'dependency'  => array( 'element' => 'image_hover_type1', 'value' => array( 'rectangle-info' ) ),
						    'group'       => esc_html__( 'Additional Features', 'eltdf-core' )
					    )
				    )
			    )
    ...
    */
	...
	public function getTextHolderStyles($params) {
		...
		if (!empty($params['content_top_margin']) && $params['info_position'] === 'below-image') {
			$styles[] = 'margin-top: '.superfood_elated_filter_px($params['content_top_margin']).'px';
		}
		
		if (!empty($params['content_bottom_margin']) && $params['info_position'] === 'below-image') {
			$styles[] = 'margin-bottom: '.superfood_elated_filter_px($params['content_bottom_margin']).'px';
		}
    ...
    */
	public function getItemCategoriesHtml($params){
		...
		$color = '';
		if(!empty($params['overlay_content_color1']) && $params['image_hover_type1'] === 'overlay-background') {
			$color = $params['overlay_content_color1'];
		}
		...
		$category_html = '<div class="eltdf-pli-category-holder">';
			foreach ($categories as $cat) {
				$category_html .= '<a itemprop="url" class="eltdf-pli-category" href="'.esc_url(get_term_link($cat->term_id)).'" '.wp_kses($style, array('style')).'>'.esc_html($cat->name).'</a>';
			}
		$category_html .= '</div>';
    ...
    */
	...
	public function getLoadMoreStyles($params) {
		...
		if (!empty($params['load_more_top_margin'])) {
			$styles[] = 'margin-top: '.superfood_elated_filter_px($params['load_more_top_margin']).'px';
		}
