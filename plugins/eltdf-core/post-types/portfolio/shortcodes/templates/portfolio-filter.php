		...
		if(is_array($filter_categories) && count($filter_categories)){ ?>
			<ul>
				<li class="eltdf-pl-filter" data-filter="">
					<span <?php superfood_elated_inline_style($filter_styles); ?>><?php esc_html_e('Show all', 'eltdf-core')?></span>
				</li>
				<?php foreach($filter_categories as $cat) { ?>
					<li class="eltdf-pl-filter" data-filter=".portfolio-category-<?php echo esc_attr($cat->slug); ?>">
						<span <?php superfood_elated_inline_style($filter_styles); ?>><?php echo esc_html($cat->name); ?></span>
					</li>
				<?php } ?>
			</ul>
		<?php } ?>
