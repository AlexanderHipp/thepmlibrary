...
class HeaderDivided extends HeaderType
{
    ...
    public function __construct() {
        ...
        if (!is_admin()) {
            ...
            $menuAreaHeight = superfood_elated_filter_px(superfood_elated_options()->getOptionValue('menu_area_height_header_divided'));
    ...
    public function loadTemplate($parameters = array()) {
        ...
        if (superfood_elated_get_header_behaviour() == 'fixed-on-scroll') {
            $parameters['header_bottom_margin'] = 'margin-bottom:' . $this->calculateHeaderHeight() . 'px;';
        } else {
            $parameters['header_bottom_margin'] = '';
        }

        $parameters['menu_area_in_grid'] = superfood_elated_get_meta_field_intersect('menu_area_in_grid_header_divided', $id) == 'yes' ? true : false;

        $parameters = apply_filters('superfood_elated_header_divided_parameters', $parameters);
    ...
    public function calculateHeightOfTransparency() {
        ...
        if (get_post_meta($id, 'eltdf_menu_area_background_transparency_header_divided_meta', true) !== '1' && get_post_meta($id, 'eltdf_menu_area_background_transparency_header_divided_meta', true) !== '') {
            $menuAreaTransparent = true;
        } else if (superfood_elated_options()->getOptionValue('menu_area_background_transparency_header_divided') !== '1' && superfood_elated_options()->getOptionValue('menu_area_background_transparency_header_divided') !== '') {
            $menuAreaTransparent = true;
    ...
    public function getPerPageJSVariables($perPageVars) {
        ...
        if (!in_array(superfood_elated_options()->getOptionValue('header_behaviour'), array('sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up'))) {
            $perPageVars['eltdfHeaderTransparencyHeight'] = $this->headerHeight - (superfood_elated_get_top_bar_height() + $this->heightOfCompleteTransparency);
        } else {
