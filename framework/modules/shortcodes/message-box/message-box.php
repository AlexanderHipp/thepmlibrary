<?php
namespace SuperfoodElatedNamespace\Modules\Shortcodes\MessageBox;
...
class MessageBox implements ShortcodeInterface	{
	...
	public function vcMap() {
		vc_map( array(
			'name'                      => esc_html__( 'Elated Message Box', 'superfood' ),
			'base'                      => $this->base,
			...
			'params'                    => array_merge(
				array(
					array(
						...
						'value'      => array(
							esc_html__( 'Normal', 'superfood' )    => 'eltdf-mb-normal',
							esc_html__( 'With Icon', 'superfood' ) => 'eltdf-mb-with-icon'
						)
					)
				),
				...
				array(
					...
					array(
						...
						'heading'    => esc_html__( 'Close Mark Color', 'superfood' )
					),
					array(
						'type'       => 'textarea_html',
						'param_name' => 'content',
	...
	public function render($atts, $content = null) {
		...
		$html = superfood_elated_get_shortcode_module_template_part('templates/message-box', 'message-box', '', $params);
	...
	private function getHolderStyles($params){
		...
		if(!empty($params['border_width']) || !empty($params['border_color'])) {
			...
			if(!empty($params['border_width'])) {
				$styles[] = 'border-width:'.superfood_elated_filter_px($params['border_width']).'px';
			}
		}
