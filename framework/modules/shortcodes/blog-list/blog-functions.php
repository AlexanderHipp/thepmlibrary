 ...
 *
 */
if(!function_exists('superfood_elated_blog_shortcode_load_more')){
	
	function superfood_elated_blog_shortcode_load_more(){
		$return_obj = array();
		$paged = 1;
		$post_number = -1;
		...
		if($query_results->have_posts()):
			while ( $query_results->have_posts() ) : $query_results->the_post();
				$html .= superfood_elated_get_shortcode_module_template_part('templates/'.$type, 'blog-list', '', $params);
			endwhile;
			else:
				$html .= '<div class="eltdf-blog-list-messsage"><p>'. esc_html__('No posts were found.', 'superfood') .'</p></div>';
			endif;
...
add_action('wp_ajax_nopriv_superfood_elated_blog_shortcode_load_more', 'superfood_elated_blog_shortcode_load_more');
add_action( 'wp_ajax_superfood_elated_blog_shortcode_load_more', 'superfood_elated_blog_shortcode_load_more' );