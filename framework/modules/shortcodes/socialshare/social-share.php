<?php
namespace SuperfoodElatedNamespace\Modules\Shortcodes\SocialShare;
...
class SocialShare implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		vc_map(array(
			'name' => esc_html__('Elated Social Share', 'superfood'),
			'base' => $this->getBase(),
			...
			'params' => array(
				array(
					...
					'value'      => array(
						esc_html__('List', 'superfood') => 'list',
						esc_html__('Dropdown', 'superfood') => 'dropdown'
					)
				),
				array(
					'type'       => 'dropdown',
					'param_name' => 'icon_type',
					'heading'    => esc_html__('Icons Type', 'superfood'),
					'value'      => array(
						esc_html__('Font Awesome', 'superfood') => 'font-awesome',
						esc_html__('Font Elegant', 'superfood') => 'font-elegant'
					)
				),
				array(
					'type'       => 'textfield',
					'param_name' => 'title',
					'heading'    => esc_html__('Social Share Title', 'superfood'),
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (superfood_elated_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (superfood_elated_options()->getOptionValue('enable_social_share_on_'.$post_type) == 'yes') ? true : false;
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= superfood_elated_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (superfood_elated_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (superfood_elated_options()->getOptionValue($net.'_icon')) ? superfood_elated_options()->getOptionValue($net.'_icon') : '';
				$html = superfood_elated_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
			}
		...
		switch ($net) {
			case 'facebook':
				if(wp_is_mobile()) {
					...
					$link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(superfood_elated_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '&amp;p[summary]=' . urlencode(superfood_elated_addslashes(get_the_excerpt())) . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
				}
				break;
			case 'twitter':
				$count_char = (isset($_SERVER['https'])) ? 23 : 22;
				$twitter_via = (superfood_elated_options()->getOptionValue('twitter_via') !== '') ? ' via ' . superfood_elated_options()->getOptionValue('twitter_via') . ' ' : '';
				if(wp_is_mobile()) {
					$link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(superfood_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
				} else {
					$link = 'window.open(\'http://twitter.com/home?status=' . urlencode(superfood_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
				}
				break;
			case 'google_plus':
			...
			case 'tumblr':
				...
				break;
			case 'pinterest':
				$link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . superfood_elated_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'vk':
