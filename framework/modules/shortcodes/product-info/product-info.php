<?php
namespace SuperfoodElatedNamespace\Modules\Shortcodes\ProductInfo;
...
class ProductInfo implements ShortcodeInterface {
	...
	public function vcMap() {
        vc_map( array(
            'name' => esc_html__('Elated Product Info', 'superfood'),
            'base' => $this->getBase(),
            ...
            'params' => array(
	                array(
		                ...
		                'heading'    => esc_html__('Selected Product', 'superfood'),
		                'settings'   => array(
			                'sortable'      => true,
			                'unique_values' => true
		                ),
	                ...
	                array(
		                ...
		                'heading'    => esc_html__('Product Info Type', 'superfood'),
		                'value'      => array(
					        esc_html__('Title', 'superfood') => 'title',
					        esc_html__('Featured Image', 'superfood') => 'featured_image',
					        esc_html__('Category', 'superfood') => 'category',
					        esc_html__('Excerpt', 'superfood') => 'excerpt',
					        esc_html__('Price', 'superfood') => 'price',
					        esc_html__('Rating', 'superfood') => 'rating',
					        esc_html__('Add To Cart', 'superfood') => 'add_to_cart',
					        esc_html__('Tag', 'superfood') => 'tag',
					        esc_html__('Author', 'superfood') => 'author',
	                ...
	                array(
		                ...
		                'heading'    => esc_html__('Product Info Color', 'superfood'),
	                ...
	                array(
		                ...
		                'save_always' => true,
		                'description' => esc_html__('Set title tag for product title element', 'superfood'),
	                ...
	                array(
		                ...
		                'heading'    => esc_html__('Featured Image Proportions', 'superfood'),
		                'value'      => array(
	                ...
	                array(
		                ...
		                'heading'     => esc_html__('Category Tag', 'superfood'),
		                'value'       => array_flip(superfood_elated_get_title_tag(true)),
		                'save_always' => true,
		                'description' => esc_html__('Set category tag for product category element', 'superfood'),
	                ...
	                array(
		                ...
		                'heading'    => esc_html__('Add To Cart Skin', 'superfood'),
		                'value'      => array(
			                esc_html__('Default', 'superfood') => '',
	...
	public function getItemTitleHtml($params){
		...
		if(!empty($title)) {
			$html = '<'.esc_attr($title_tag).' itemprop="name" class="eltdf-pi-title entry-title" '.superfood_elated_get_inline_style($product_info_style).'>';
    ...
    */
			...
			$html = '<div class="eltdf-pi-category">';
				foreach ($categories as $cat) {
					if(!empty($params['category_tag'])) {
						$html .= '<'.esc_attr($params['category_tag']).' '.superfood_elated_get_inline_style($product_info_style).'>';
						...
						$html .= '<a itemprop="url" class="eltdf-pi-category-item" href="'.esc_url(get_term_link($cat->term_id)).'" '.superfood_elated_get_inline_style($product_info_style).'>'.esc_html($cat->name).'</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml($params){
		...
		if(!empty($excerpt)) {
			$html = '<div class="eltdf-pi-excerpt"><p itemprop="description" class="eltdf-pi-excerpt-item" '.superfood_elated_get_inline_style($product_info_style).'>'.esc_html($excerpt).'</p></div>';
		}
	...
	public function getItemPriceHtml($params){
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="eltdf-pi-price" '.superfood_elated_get_inline_style($product_info_style).'>'.$price_html.'</div>';
		}
	...
	public function getItemRatingHtml($params){
		...
		if (get_option('woocommerce_enable_review_rating') !== 'no') {
			...
			$html = '<div class="eltdf-pi-rating" title="'.sprintf(esc_html__("Rated %s out of 5", "superfood"), $average ).'" '.superfood_elated_get_inline_style($product_info_style).'><span style="width:'.(($average / 5)*100).'%"></span></div>';
		}
	...
	public function getItemAddToCartHtml($params){
		...
		$html .= apply_filters( 'superfood_elated_product_info_add_to_cart_link',
	...
	public function getItemTagHtml($params){
		...
		if(!empty($tags)) {
			...
			foreach ($tags as $tag) {
				$html .= '<a itemprop="url" class="eltdf-pi-tag-item" href="'.esc_url(get_term_link($tag->term_id)).'" '.superfood_elated_get_inline_style($product_info_style).'>'.esc_html($tag->name).'</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml($params){
		...
		if(!empty($author)) {
			$html = '<div class="eltdf-pi-author" '.superfood_elated_get_inline_style($product_info_style).'>'.esc_html($author).'</div>';
		}
	...
	public function getItemDateHtml($params){
		...
		if(!empty($date)) {
			$html = '<div class="eltdf-pi-date" '.superfood_elated_get_inline_style($product_info_style).'>'.esc_html($date).'</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'superfood' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'superfood' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $portfolio_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'superfood' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'superfood' ) . ': ' . $product_id;
