...
if(!function_exists('superfood_elated_get_title')) {
    /**
     * Loads title area HTML
     */
    function superfood_elated_get_title() {
        $id = superfood_elated_get_page_id();

        extract(superfood_elated_title_area_height());
        extract(superfood_elated_title_area_background());

	    
	    $show_title_area_meta = superfood_elated_get_meta_field_intersect('show_title_area');
        ...
        if(superfood_elated_is_woocommerce_installed() && is_singular('product')) {
	    ...
	    $type = superfood_elated_get_meta_field_intersect('title_area_type');
	    if(empty($type)) {
		    $type = 'standard';
	    }
        ...
        if(($meta_temp = get_post_meta($id, "eltdf_title_text_font_size_meta", true)) !== ""){
            $title_font_size = 'font-size:'.superfood_elated_filter_px($meta_temp).'px;';
        }
        ...
        $parameters = apply_filters('superfood_elated_title_area_height_params', $parameters);

        superfood_elated_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('superfood_elated_get_title_text')) {
    /**
     * Function that returns current page title text. Defines superfood_elated_title_text filter
     ...
     */
    function superfood_elated_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'superfood');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'superfood') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search results for: ', 'superfood') . get_search_query();
        }

        //is single post page
        elseif (is_singular('post')) {
            if (superfood_elated_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
        ...
        elseif (is_404()) {
            ...
            if(superfood_elated_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = superfood_elated_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'superfood');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(superfood_elated_is_woocommerce_installed() && (superfood_elated_is_woocommerce_shop() || is_singular('product'))) {
        ...
        elseif(superfood_elated_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','superfood');
        }
        ...
        $title = apply_filters('superfood_elated_title_text', $title);

        return $title;
    }
}

if(!function_exists('superfood_elated_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see superfood_elated_get_title_text()
     */
    function superfood_elated_title_text() {
        echo superfood_elated_get_title_text();
    }
}

if(!function_exists('superfood_elated_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function superfood_elated_subtitle_text() {
        $id = superfood_elated_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('superfood_elated_custom_breadcrumbs')) {
    /**
     * Function that renders breadcrumbs
     */
    function superfood_elated_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = superfood_elated_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="eltdf-delimiter" '.superfood_elated_get_inline_style($bread_style).'></span>'; // delimiter between crumbs
        $home = esc_html__('Home', 'superfood'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="eltdf-current" '.superfood_elated_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="eltdf-breadcrumbs"><div class="eltdf-breadcrumbs-inner" itemprop="breadcrumbs"><a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="eltdf-breadcrumbs"><div class="eltdf-breadcrumbs-inner" itemprop="breadcrumbs"><a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="eltdf-breadcrumbs" itemprop="breadcrumbs"><a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || superfood_elated_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'superfood') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
            ...
            } elseif ( superfood_elated_is_woocommerce_installed() && is_singular('product') ){
                global $product;
                if(get_option('woocommerce_shop_page_id')){
                    $output .= '<a itemprop="url" '. superfood_elated_get_inline_style($bread_style) .' href="' . get_permalink(get_option('woocommerce_shop_page_id')) . '">' . get_the_title(get_option('woocommerce_shop_page_id')) . '</a>' . $delimiter;
                }
               ...
               if(wc_get_product_category_list($product->get_id(), ', ')) {
                ...
                if (!empty($portfolio_categories) && count($portfolio_categories)) {
                    foreach($portfolio_categories as $cat) {
                        $output .= '<a itemprop="url" '. superfood_elated_get_inline_style($bread_style) .' href="'. get_term_link($cat->term_id) .'">'.$cat->name.'</a>' . $delimiter;
                    }
                }
                ...
                if($post->post_parent) {
                    ...
                    $output .= '<a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                if($post->post_parent) {
                    ...
                    foreach($parent_ids as $parent_id) {
                        $output .= '<a itemprop="url" '.superfood_elated_get_inline_style($bread_style).' href="' . get_the_permalink($parent_id) . '">' . get_the_title($parent_id) . '</a>' . $delimiter;
                    }
                }
                ...
                $output .= $before . esc_html__('Posts tagged ','superfood') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                $author_id = get_query_var('author');
                $output .= $before . esc_html__('Articles posted by ','superfood') . get_the_author_meta('display_name', $author_id) . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404','superfood') . $after;

            } elseif(superfood_elated_is_woocommerce_installed() && superfood_elated_is_woocommerce_shop()){
                $shop_id = get_option('woocommerce_shop_page_id');
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'superfood') . ' ' . get_query_var('paged') . ")" . $after;
            }
...
if(!function_exists('superfood_elated_get_title_area_height')) {
    /**
     * Function that returns title height
     **/
    function superfood_elated_get_title_area_height() {
        $title_height = superfood_elated_get_meta_field_intersect('title_area_height');
	    if(empty($title_height)) {
		    $title_height = 184;
	    }

        return apply_filters('superfood_elated_title_area_height', $title_height);
    }
}

if(!function_exists('superfood_elated_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/
    function superfood_elated_get_title_content_padding() {
        return apply_filters('superfood_elated_title_content_padding', 0);
    }
}

if(!function_exists('superfood_elated_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/
    function superfood_elated_title_area_height() {
        $id = superfood_elated_get_page_id();
        $title_height_and_padding = array();
        $title_height          = superfood_elated_get_title_area_height();
        $header_height_padding = superfood_elated_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "eltdf_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = superfood_elated_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "eltdf_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = superfood_elated_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('superfood_elated_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/
    function superfood_elated_title_area_background() {
        $id = superfood_elated_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "eltdf_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = superfood_elated_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "eltdf_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = superfood_elated_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = superfood_elated_get_image_dimensions($background_image);
