...
class ElatedSkin extends SuperfoodElatedClassSkinAbstract {
    /**
     * Skin constructor. Hooks to superfood_elated_admin_scripts_init and superfood_elated_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('superfood_elated_admin_scripts_init', array($this, 'registerStyles'));
        add_action('superfood_elated_admin_scripts_init', array($this, 'registerScripts'));

        add_action('superfood_elated_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('superfood_elated_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('superfood_elated_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('superfood_elated_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            superfood_elated_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            superfood_elated_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = superfood_elated_get_admin_tab();
        $active_page = $superfood_elated_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param SuperfoodElatedAdminPage $page current page to load
     *
     * @see ElatedSkinAbstract::loadTemplatePart()
     */
